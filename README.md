...
Con todo esto configurado podremos ejecutar **provider**:

```
vagrant up --provider=azure
```
Con este comando se crea la máquina Azure a la misma vez que se configura y se ejecuta.

Si ya tenemos la máquina creada, usaremos el comando:

```
vagrant provision
```

Como he puesto antes, he creado un script para automatizar todas las tareas:
